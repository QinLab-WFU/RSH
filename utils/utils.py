...
class CrossModalCenterHelper(nn.Module):
    def __init__(self, n_classes, n_bits):
        super().__init__()
        self.register_buffer("img_centers", torch.zeros(n_classes, n_bits))
        self.register_buffer("txt_centers", torch.zeros(n_classes, n_bits))
        self.last_col_labs = None

    def update(self, row_img_embs, row_txt_embs, row_labs):
        if self.last_col_labs is None:
            return

        for c in self.last_col_labs:
            mask = row_labs[:, c] == 1
            if mask.sum() > 0:
                self.img_centers[c].copy_(row_img_embs[mask].mean(0))
                self.txt_centers[c].copy_(row_txt_embs[mask].mean(0))

    def set_last_col_labs(self, col_labs):
        self.last_col_labs = (col_labs.sum(0) > 0).nonzero(as_tuple=True)[0]

    @property
    def centers(self):
        valid = ((self.img_centers.abs().sum(1) > 0) & (self.txt_centers.abs().sum(1) > 0)).nonzero(as_tuple=True)[0]
        center_labs = F.one_hot(valid, self.img_centers.size(0)).float()
        return self.img_centers[valid], self.txt_centers[valid], center_labs


class CenterHelper(nn.Module):
    def __init__(self, n_classes, n_bits):
        super().__init__()

        self.register_buffer("center_embs", torch.zeros(n_classes, n_bits))
        self.last_col_labs = None

    def update(self, row_embs, row_labs):
        if self.last_col_labs is None:
            return

        for x in self.last_col_labs:
            mask = row_labs[:, x] == 1
            if mask.sum() > 0:
                center_emb = row_embs[mask].mean(0)
                self.center_embs[x].copy_(center_emb)
    ...
    @property
    def centers(self):
        idxes = (self.center_embs.abs().sum(1) > 0).nonzero(as_tuple=True)[0]
        return self.center_embs[idxes], F.one_hot(idxes, self.center_embs.size(0)).float()
