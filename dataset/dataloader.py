from .base import BaseDataset, BaseDataset1, BaseDataset3
import numpy as np
...
def dataloader1(captionFile: str,
               indexFile: str,
               labelFile: str,
               maxWords=32,
               imageResolution=224,
    ...
    if return_combined:
        # 返回完整数据集
        full_data = BaseDataset(captions=captions, indexs=indexs, labels=labels,
                                maxWords=maxWords, imageResolution=imageResolution, npy=npy)
...
def dataloader2(captionFile: str,
               indexFile: str,
               labelFile: str,
               maxWords=32,
               imageResolution=224,
    ...
    # 创建训练数据时传入噪声比例
    train_data = BaseDataset1(captions=split_captions[1], indexs=split_indexs[1], labels=split_labels[1],
                             maxWords=maxWords, imageResolution=imageResolution,
                             noise_ratio=noise_ratio, npy=npy)  # 添加噪声参数
    query_data = BaseDataset1(captions=split_captions[0], indexs=split_indexs[0], labels=split_labels[0],
                             maxWords=maxWords, imageResolution=imageResolution, is_train=False, npy=npy)
    retrieval_data = BaseDataset1(captions=split_captions[2], indexs=split_indexs[2], labels=split_labels[2],
                                 maxWords=maxWords, imageResolution=imageResolution, is_train=False, npy=npy)

    return train_data, query_data, retrieval_data


def dataloader3(captionFile: str,
               indexFile: str,
               labelFile: str,
               maxWords=32,
               imageResolution=224,
    ...
    train_data = BaseDataset3(
        ...
        npy=npy,
        noise_ratio=noise_ratio,  # 噪声比例
        noise_file_path=noise_file_path,  # 噪声文件路径
        is_train=True  # 只在训练集添加噪声
    )

    query_data = BaseDataset3(
